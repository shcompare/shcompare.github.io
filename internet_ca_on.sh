...
curl -s 'https://www.rogers.com/web/totes/browsebuy/sai/v2/getSAIOffers' \
	...
	| jq ".en.bundleOffers[].internetOffers[] | {isp:\"rogers\", down:((.speed.download | tostring) + \" \" + .speed.downloadUnit), up:((.speed.upload|tostring) + \" \" + .speed.uploadUnit), price:.offerPrice.price, cap:.usageAllowance}"
...
curl -s 'https://package.api.teksavvy.com/api/v1/External/Qualification/Packages?classificationID=1&uaid=C7FC6861-7189-2B8E-4FC7-20FD8B6C5F76' \
	| jq ".[] | select(.serviceType==\"Internet\") | select(.qualifies==true) | .packages[] | {isp:\"teksavvy\", cap:.bandwidth, down:.download, up:.upload, price:.price}"
...
while read -r URL ; do
	curl -s "$URL" > "$TMP"
	echo -e "{\n  \"isp\":\"primus\","
	grep 'class="promo"' "$TMP" | perl -pe 's/.*\$/  "price":/g;s/<.*/,/g'
	C=1;
	while read -r V ; do
		if [ "$C" -eq 1 ] ; then
			echo "  \"down\":\"$V\","
		...
		C=$((C+1))
	done < <(grep "load speed" "$TMP" | perl -pe 's/(bps) .*/$1/g;s/.* to //g')
	echo "}"
